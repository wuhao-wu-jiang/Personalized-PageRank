...
```
After the program returned, there should be 3 output files in "*DataSetCleaned/dblp*". 

1. **attribute.txt** : a meta file that records the number of vertices and edges in the output graph. 
   
2. **graph.txt** : the output graph in txt form. The first line contains a number, which is the number of vertices. Each of the following lines represents an edge by using the ids of its endpoints.  This is output only for reference.
